...
def main():
    ...
    model.load_state_dict(torch.load('./ghostnet_pytorch/models/state_dict_73.98.pth'))
...
def validate(model, loader, loss_fn, args, log_suffix=''):
    ...
    # 将模型权重转为dict格式（key=str, value=np.array），保存dict到文件
    model_param = dict()
    # for k,v in list(model.named_parameters()):
    for k, v in model.state_dict().items():
        model_param[k] = v.cpu().numpy()

    import numpy as np
    np.save('model_param2.npy', model_param)
